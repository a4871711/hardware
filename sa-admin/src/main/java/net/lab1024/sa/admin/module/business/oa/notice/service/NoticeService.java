...
public class NoticeService {
    ...
    private ResponseDTO<String> checkAndBuildVisibleRange(NoticeAddForm form) {
        ...
        if (CollectionUtils.isNotEmpty(employeeIdList)) {
            ...
            List<Long> dbEmployeeIdList = employeeDao.selectBatchIds(employeeIdList).stream().map(EmployeeBaseEntity::getEmployeeId).collect(Collectors.toList());
        ...
        if (CollectionUtils.isNotEmpty(deptIdList)) {
            ...
            List<Long> dbDeptIdList = departmentDao.selectBatchIds(deptIdList).stream().map(DepartmentBaseEntity::getDepartmentId).collect(Collectors.toList());
    ...
    public NoticeUpdateFormVO getUpdateFormVO(Long noticeId) {
        ...
        if (!updateFormVO.getAllVisibleFlag()) {
            ...
            Map<Long, EmployeeBaseEntity> employeeMap = null;
            if (CollectionUtils.isNotEmpty(employeeIdList)) {
                employeeMap = employeeDao.selectBatchIds(employeeIdList).stream().collect(Collectors.toMap(EmployeeBaseEntity::getEmployeeId, Function.identity()));
            } else {
            ...
            for (NoticeVisibleRangeVO noticeVisibleRange : noticeVisibleRangeList) {
                if (noticeVisibleRange.getDataType().equals(NoticeVisibleRangeDataTypeEnum.EMPLOYEE.getValue())) {
                    EmployeeBaseEntity employeeEntity = employeeMap.get(noticeVisibleRange.getDataId());
