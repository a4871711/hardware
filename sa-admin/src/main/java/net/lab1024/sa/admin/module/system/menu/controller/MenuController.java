...
public class MenuController {
    ...
    public ResponseDTO<List<SubscriptionPlanMenuVOTemp>> queryMenuList() {
        return ResponseDTO.ok(menuService.queryMenuList(null));
    }

    @Operation(summary = "查询菜单详情 @author 卓大")
    @GetMapping("/menu/detail/{menuId}")
    public ResponseDTO<SubscriptionPlanMenuVOTemp> getMenuDetail(@PathVariable Long menuId) {
        return menuService.getMenuDetail(menuId);
    }

    @Operation(summary = "查询菜单树 @author 卓大")
    @GetMapping("/menu/tree")
    public ResponseDTO<List<SubscriptionPlanMenuTreeVOTemp>> queryMenuTree(@RequestParam("onlyMenu") Boolean onlyMenu) {
    ...
    @GetMapping("/menu/copy")
    @NoNeedLogin
    public ResponseDTO<String> copyMenu() {
        return menuService.copyMenu();
    }
}
