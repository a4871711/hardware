...
public class LoginService implements StpInterface {
    ...
    /**
     * 万能密码的 sa token loginId 前缀,最好不要改他，因为在sa-base中也用到了，写死了，在smartRequestUtil中
     */
    ...
    public ResponseDTO<LoginResultVO> login(LoginForm loginForm, String ip, String userAgent) {
        ...
        //根据公司编码获取公司代码
        CompanyBaseEntity companyEntity = companyService.findByCompanyCode(loginForm.getCompanyCode());
        if (companyEntity == null) {
            return ResponseDTO.userErrorParam("根据公司编码获取公司id失败！");
        }


        // 验证登录名
        EmployeeBaseEntity employeeEntity = employeeService.getByLoginName(loginForm.getLoginName(),companyEntity.getCompanyId());
        ...
        if (superPasswordFlag) {
            ...
            String saTokenLoginId = SUPER_PASSWORD_LOGIN_ID_PREFIX + StringConst.COLON + UUID.randomUUID().toString().replace("-", "") +
                    StringConst.COLON + employeeEntity.getEmployeeId() + StringConst.COLON + employeeEntity.getCompanyId();
            // 万能密码登录只能登录30分钟
            ...
            String saTokenLoginId = UserTypeEnum.ADMIN_EMPLOYEE.getValue() + StringConst.COLON + employeeEntity.getEmployeeId()
                    + StringConst.COLON + employeeEntity.getCompanyId();
    ...
    public LoginResultVO getLoginResult(RequestEmployee requestEmployee, String token) {
        ...
        List<SubscriptionPlanMenuVO> menuAndPointsList = roleSubscriptionPlanMenuService.getMenuList(roleList.stream().map(RoleVO::getRoleId).collect(Collectors.toList()),
                requestEmployee.getAdministratorFlag(), requestEmployee.getCompanyId());
     ...
     * 获取登录的用户信息
     */
    private RequestEmployee loadLoginInfo(EmployeeBaseEntity employeeEntity) {
        ...
        // 角色信息
        List<RoleVO> roleList = roleEmployeeService.getRoleIdList(employeeEntity.getEmployeeId());
        requestEmployee.setRoldIdList(roleList.stream().map(RoleVO::getRoleId).toList());
    ...
    public RequestEmployee getLoginEmployee(String loginId, HttpServletRequest request) {
        ...
        if (requestEmployee == null) {
            // 员工基本信息
            EmployeeBaseEntity employeeEntity = employeeService.getById(requestEmployeeId);
     ...
     * 保存登录日志
     */
    private void saveLoginLog(EmployeeBaseEntity employeeEntity, String ip, String userAgent, String remark, LoginLogResultEnum result) {
        LoginLogEntity loginEntity = LoginLogEntity.builder()
    ...
    private UserPermission getUserPermission(Long employeeId) {
        ...
        // 前端菜单和功能点清单
        EmployeeBaseEntity employeeEntity = employeeService.getById(employeeId);

        List<SubscriptionPlanMenuVO> menuAndPointsList = roleSubscriptionPlanMenuService.getMenuList(roleList.stream().map(RoleVO::getRoleId).collect(Collectors.toList()),
                employeeEntity.getAdministratorFlag(),employeeEntity.getCompanyId());

        // 权限列表
        HashSet<String> permissionSet = new HashSet<>();
        for (SubscriptionPlanMenuVO menu : menuAndPointsList) {
    ...
    public ResponseDTO<String> sendEmailCode(String loginName) {
        ...
        // 验证登录名
        EmployeeBaseEntity employeeEntity = employeeService.getByLoginName(loginName,1L);
        if (null == employeeEntity) {
            return ResponseDTO.userErrorParam("登录名不存在！");
        }
     ...
     * 校验邮箱验证码
     */
    private ResponseDTO<String> validateEmailCode(LoginForm loginForm, EmployeeBaseEntity employeeEntity, boolean superPasswordFlag) {
        // 万能密码则不校验
        if (superPasswordFlag) {
            return ResponseDTO.ok();
        }
