...
import net.lab1024.sa.admin.module.system.position.domain.entity.PositionBaseEntity;
...
public class EmployeeService {
    ...
    public EmployeeBaseEntity getById(Long employeeId) {
    ...
    public ResponseDTO<PageResult<EmployeeVO>> queryEmployee(EmployeeQueryForm employeeQueryForm) {
        ...
        List<PositionBaseEntity> positionEntityList = positionIdList.isEmpty() ? Collections.emptyList() : positionDao.selectBatchIds(positionIdList);
        Map<Long, String> positionNameMap = positionEntityList.stream().collect(Collectors.toMap(PositionBaseEntity::getPositionId, PositionBaseEntity::getPositionName));
    ...
    public synchronized ResponseDTO<String> addEmployee(EmployeeAddForm employeeAddForm) {
        Long requestCompanyId = SmartRequestUtil.getRequestCompanyId();
        if(requestCompanyId == null) {
            requestCompanyId = employeeAddForm.getEmployeeId();
        }

        // 校验登录名是否重复
        EmployeeBaseEntity employeeEntity = employeeDao.getByLoginName(employeeAddForm.getLoginName(), null,requestCompanyId);
        if (null != employeeEntity) {
            return ResponseDTO.userErrorParam("登录名重复");
        }

        EmployeeBaseEntity entity = SmartBeanUtil.copy(employeeAddForm, EmployeeBaseEntity.class);
    ...
    public synchronized ResponseDTO<String> updateEmployee(EmployeeUpdateForm employeeUpdateForm) {
        ...
        EmployeeBaseEntity employeeEntity = employeeDao.selectById(employeeId);
        if (null == employeeEntity) {
        ...
        DepartmentBaseEntity departmentEntity = departmentDao.selectById(departmentId);
        if (departmentEntity == null) {
            return ResponseDTO.userErrorParam("部门不存在");
        }


        EmployeeBaseEntity existEntity = employeeDao.getByLoginName(employeeUpdateForm.getLoginName(), null, SmartRequestUtil.getRequestCompanyId());
        ...
        // 不更新密码
        EmployeeBaseEntity entity = SmartBeanUtil.copy(employeeUpdateForm, EmployeeBaseEntity.class);
    ...
    public ResponseDTO<String> updateAvatar(EmployeeUpdateAvatarForm employeeUpdateAvatarForm) {
        ...
        // 更新头像
        EmployeeBaseEntity updateEntity = new EmployeeBaseEntity();
    ...
    public ResponseDTO<String> batchUpdateDeleteFlag(List<Long> employeeIdList) {
        ...
        List<EmployeeBaseEntity> employeeEntityList = employeeManager.listByIds(employeeIdList);
        if (CollectionUtils.isEmpty(employeeEntityList)) {
            return ResponseDTO.ok();
        }
        // 更新删除
        List<EmployeeBaseEntity> deleteList = employeeIdList.stream().map(e -> {
            EmployeeBaseEntity updateEmployee = new EmployeeBaseEntity();
    ...
    public ResponseDTO<String> batchUpdateDepartment(EmployeeBatchUpdateDepartmentForm batchUpdateDepartmentForm) {
        ...
        List<EmployeeBaseEntity> employeeEntityList = employeeDao.selectBatchIds(employeeIdList);
        if (employeeIdList.size() != employeeEntityList.size()) {
            return ResponseDTO.error(UserErrorCode.DATA_NOT_EXIST);
        }
        // 更新
        List<EmployeeBaseEntity> updateList = employeeIdList.stream().map(e -> {
            EmployeeBaseEntity updateEmployee = new EmployeeBaseEntity();
            updateEmployee.setEmployeeId(e);
    ...
    public ResponseDTO<List<EmployeeVO>> getAllEmployeeByDepartmentId(Long departmentId, Boolean disabledFlag) {
        List<EmployeeBaseEntity> employeeEntityList = employeeDao.selectByDepartmentId(departmentId, disabledFlag);
     ...
     * 根据登录名获取员工
     */
    public EmployeeBaseEntity getByLoginName(String loginName, Long companyId) {
        return employeeDao.getByLoginName(loginName, null,companyId);
    }
