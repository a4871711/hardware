 ...
 */
@Service
public class FileService extends ServiceImpl<FileDao, FileBaseEntity> {
    ...
    public ResponseDTO<FileUploadVO> fileUpload(MultipartFile file, Integer folderType, RequestUser requestUser) {
        ...
        String path = folderTypeEnum.getFolder() + companyId + "/";
        // 进行上传
        ResponseDTO<FileUploadVO> response = fileStorageService.upload(file, path);
        if (!response.getOk()) {
            return response;
        }
        ...
        FileBaseEntity fileEntity = new FileBaseEntity();
    ...
    public List<FileBO> getFileByIds(List<Long> fileIds) {
        if (CollectionUtils.isEmpty(fileIds)) {
            return null;
        }
        List<FileBO> fileBOList = Lists.newArrayList();

        LambdaQueryWrapper<FileBaseEntity> wrapper = new LambdaQueryWrapper<>();
        wrapper.in(FileBaseEntity::getFileId, fileIds);

        List<FileBaseEntity> fileEntityList = fileDao.selectList(wrapper);
        for (FileBaseEntity fileEntity : fileEntityList) {
            FileBO fileBO = SmartBeanUtil.copy(fileEntity, FileBO.class);
            ResponseDTO<String> fileUrlResponse = fileStorageService.getFileUrl(fileEntity.getFileKey());
            if (Boolean.TRUE.equals(fileUrlResponse.getOk())) {
                fileBO.setFileUrl(fileUrlResponse.getData());
            }
